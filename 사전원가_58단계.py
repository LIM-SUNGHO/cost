...
# 로그 설정
logging.basicConfig(filename="log_BOM_사전원가_조제.log", level=logging.INFO, 
...
def process_file(file_name):
    try:
        ...
        # 0) '품목대분류'가 '조제'인 데이터만 추출
        bom_df = bom_df[bom_df['품목대분류'] == '조제']
        logging.info("'품목대분류'가 '조제'인 데이터만 추출하였습니다.")
        ...
        # 2) '구분.1' 헤더 생성 후 '공정흐름차수명' 값을 채움
        bom_df['구분.1'] = bom_df['공정흐름차수명']

        # 3) '구분.1' 값이 '0'인 경우 '포장'으로 변경
        bom_df.loc[bom_df['구분.1'] == '0', '구분.1'] = '포장'
        ...
        조건2 = bom_df['자재명'].str.contains('배전-|미세-', na=False)
        ...
        # 9) 헤더 순서 조정
        selected_columns = [
            '구분','품목대분류', '품번', '품명', '규격', '단위', '구분.1', '공정.1', '항목',
            '자재번호', '자재명', '자재규격', '단위.2', '사전원가'
        ]
        bom_df = bom_df[selected_columns]

        # 10) 헤더 이름 변경
        bom_df.rename(columns={'공정.1': '공정', '단위.2': '자재단위','구분' : '비고','구분.1': '구분'}, inplace=True)

        # 11) '품목대분류' 값이 '조제'인 행을 위로 이동
        bom_df = pd.concat([
            bom_df[bom_df['품목대분류'] == '조제'],
            bom_df[bom_df['품목대분류'] != '조제']
        ...
        logging.info(f"파일 '{file_name}' 처리 완료. 결과 저장: '{file_name}'")
        print(f"파일 '{file_name}' 처리 완료. 결과 저장: '{file_name}'")
